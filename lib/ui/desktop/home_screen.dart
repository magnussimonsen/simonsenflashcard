...
/// Desktop: entry screen showing available decks.
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _openFromList(BuildContext context) async {
    ...
    _loadAndNavigate(context, path);
  }

  Future<void> _importDeck(BuildContext context) async {
    final initialDir = await DeckService.getDecksRootPath();
    ...
    if (path == null) return;
    if (!context.mounted) return;
    await _loadAndNavigate(context, path);
    _refreshDecks();
  }

  Future<void> _loadAndNavigate(BuildContext context, String path) async {
    try {
          ...
          children: [
            ...
            if (_loading)
              ...
              ElevatedButton.icon(
                ...
                onPressed: () => _openFromList(context),
              ),
            const SizedBox(height: 16),
            ElevatedButton.icon(
              icon: const Icon(Icons.download),
              label: const Text('Import deck from folder'),
